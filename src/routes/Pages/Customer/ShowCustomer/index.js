  ...
  return (
    <PageContainer heading={<IntlMessages id={heading} />} className={styles.container}>
      ...
      <Box className={classes.root}>
        <Paper className={classes.paper}>
          <div className={styles.contentArea}>
            <div className={styles.title}>CÁC SẢN PHẨM ĐÃ MUA</div>
            <Row justify="space-between">
              {orders.map(order => {
                count += 1;
                ...
                return (
                  <>
                      {orderDetails.map(detail => {
                        return (
                          <>
                            <CardInput title="Tên sản phẩm" width={'19%'}>
                              <Input type="text" value={detail.product.productName} />
                            </CardInput>
                            <CardInput title="Số lượng sản phẩm" width={'19%'}>
                              <Input type="text" value={detail.quantityOrder} />
                            </CardInput>
                            <CardInput title="Giá sản phẩm" width={'19%'}>
                              <Input type="text" value={detail.priceEach.toLocaleString('vi-VN')} />
                            </CardInput>
                            <CardInput title="Tổng đơn" width={'19%'}>
                              <Input type="text" value={(detail.quantityOrder * detail.priceEach).toLocaleString("vi-VN")} />
                            </CardInput>
                            </>
                        );
                      })}
