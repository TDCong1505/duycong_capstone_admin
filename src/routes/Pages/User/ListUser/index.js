            ...
            content={
              <div>
                <div style={{ padding: '5px' }}>
                  <Link to={`${rootPath.listUser}/show/${record.id.props.children}`}>Xem chi tiết</Link>
                </div>
                <div style={{ padding: '5px' }}>
                  <Link to={`${rootPath.listUser}/edit/${record.id.props.children}`}>Sửa thông tin</Link>
                </div>
              </div>
            }
            trigger="click">
...
export default function ListUser() {
  ...
  const pathCreate = `${rootPath.listUser}/new`;
  ...
  list.forEach(item => {
    const JobDetailCpn = ({ children }) => (
      <Link style={{ color: 'black' }} to={`${rootPath.listUser}/show/${item.id}`}>
        {children}
      </Link>
    );
    rows.push({
