...
const Map = ({ handlePostAddress, handleCloseModalMap }) => {
  //   const { handlePostAddress, handleCloseModalMap, handleChangeMainAddress } = props;
  const initLocation = {};
  const handleChangeMainAddress = null;
  const { FjobProvince: provinceList, FjobDistrict: districtList, FjobCommune: communeList } = useSelector(
    state => state.initData.initData || {},
  );
  const { latitude = 21.0377606916171, longitude = 105.83626031063875 } = JSON.parse(
    localStorage.getItem(StorageConstants.localStorage.geolocation) || '{}',
  );

  const [viewport, setViewport] = useState({
    latitude: 21.0377606916171,
    longitude: 105.83626031063875,
    zoom: 14,
  });
  ...
  const [selected, setSelected] = useState(initLocation ?? undefined);
  ...
  const onPressSearch = async () => {
    ...
    if (txtSearch.trim()) {
      const url = `https://rsapi.goong.io/Place/AutoComplete?api_key=${
        process.env.REACT_APP_GOONG_KEY_API
  ...
  const onSelectAddress = async item => {
    const url = `https://rsapi.goong.io/geocode?place_id=${item.place_id}&api_key=${process.env.REACT_APP_GOONG_KEY_API}`;
    ...
    if (resData?.results?.[0]) {
      ...
      const provinceId =
        provinceList.find(i => i.name.lastIndexOf(addressDetail.address_components?.[index]?.short_name) >= 0)?.id || 0;
      const districtId =
        districtList.find(
          i =>
  ...
  return (
    <div className={`map ${styles.map}`}>
      ...
      <ReactMapGL
        ...
        goongApiAccessToken={process.env.REACT_APP_GOONG_KEY_MAP}
        width="100%"
        height={400}
        // onLoad={onMapLoad}
